...
class UrlCrypt
{
    ...
    public static function DecryptAsArray($str, $needle, $needle2)
    {
        $r = array();
        $tstr = UrlCrypt::decrypt($str);
        $t = explode($needle, $tstr);
        for ($i = 0; $i < count($t); $i++)
        {
           $pos= strpos($t[$i],$needle2);
            $src = substr($t[$i],0,$pos);
            $dest =substr($t[$i],$pos+1);
            $r+=array($src=>$dest);
