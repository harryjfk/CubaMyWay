...
class  t_destripRow extends TDataRow
{
    public function  __construct($table)
    {
        parent::_construct($table);
...
class  t_realtripRow extends TDataRow
{
    ...
    public function  dateinitrip($value = null)
    {
        if ($value == null) {
            return $this->GetItem('dateinitrip');
        } else {
            $this->SetItem('dateinitrip', $value);
        }
    }

    public function  datefintrip($value = null)
    {
        if ($value == null) {
            return $this->GetItem('datefintrip');
        } else {
            $this->SetItem('datefintrip', $value);
        }
    }
...
class  t_descrealtripRow extends TDataRow
{
    public function  __construct($table)
    {
        parent::_construct($table);
    ...
    public function  idrealtripdesc($value = null)
    {
        if ($value == null) {
            return $this->GetItem('idrealtripdesc');
        } else {
            $this->SetItem('idrealtripdesc', $value);
        }
    }
...
class t_commentDataTable extends TDataTable
{
    ...
    private function  InitClass()
    {
        ...
        $this->columnpictureuser->AutoIncrementStep(1);
        ...
        $this->columnpictureuser->MaxLength(-1);
        $this->columnpictureuser->Caption("pictureuser");
        $this->columnpictureuser->DefaultValue('<DBNull>');
    ...
    public function  Addt_commentRow($iduser, $topiccomment, $textcomment, $nameuser, $username, $pictureuser)
    {
        $row = new Row($this);
...
class t_messagesDataTable extends TDataTable
{
    ...
    private function  InitClass()
    {
        ...
        $this->columndestuserpic->AutoIncrementStep(1);
        ...
        $this->columndestuserpic->MaxLength(-1);
        $this->columndestuserpic->Caption("destuserpic");
        $this->columndestuserpic->DefaultValue('<DBNull>');
        ...
        $this->columnoriguserpic->AutoIncrementStep(1);
        ...
        $this->columnoriguserpic->MaxLength(-1);
        $this->columnoriguserpic->Caption("origuserpic");
        $this->columnoriguserpic->DefaultValue('<DBNull>');
    ...
    public function  destuserpiccolumn()
    {
        return $this->columndestuserpic;
    }

    public function  origuserpiccolumn()
    {
        return $this->columnoriguserpic;
    }
    ...
    public function  Addt_messagesRow($idoriguser, $iddestuser, $textmsg, $origuser, $destuser, $topicmsg, $statemsg, $datemsg, $destuserpic, $origuserpic)
    {
        $row = new Row($this);
        ...
        $row->datemsg($datemsg);
        $row->destuserpic($destuserpic);
        $row->origuserpic($origuserpic);
        $this->Rows()->Add($row);
        return $row;
    }
...
class t_destripDataTable extends TDataTable
{
    ...
    private function  InitClass()
    {
        ...
        $this->columnidtourpolo->DefaultValue('<DBNull>');
        $this->Columns()->Add($this->columnidtourpolo);
        $this->DefaultType('t_destripRow');
    }

    public function  __construct()
    {
        parent::__construct();
        $this->SetTableName("t_destrip");
        parent::BeginInit();
        $this->InitClass();
        parent::EndInit();
    }
    ...
    public function  Addt_destripRow($idtrip, $idtourpolo)
    {
        $row = new Row($this);
...
class t_realtripDataTable extends TDataTable
{
    ...
    private $columndateinitrip;
    private $columndatefintrip;
    private $columnnametrip;
    private $columnnameuser;
    ...
    private function  InitClass()
    {
        ...
        $this->columndateinitrip = new TDataColumn();
        $this->columndateinitrip->TDataColumn3("dateinitrip", "", null, $this);
        $this->columndateinitrip->AutoIncrement(False);
        $this->columndateinitrip->AutoIncrementStep(0);
        $this->columndateinitrip->AutoIncrementSeed(0);
        $this->columndateinitrip->AllowDBNull(True);
        $this->columndateinitrip->ReadOnly(False);
        $this->columndateinitrip->Unique(False);
        $this->columndateinitrip->MaxLength(-1);
        $this->columndateinitrip->Caption("dateinitrip");
        $this->columndateinitrip->DefaultValue('<DBNull>');
        $this->Columns()->Add($this->columndateinitrip);
        $this->columndatefintrip = new TDataColumn();
        $this->columndatefintrip->TDataColumn3("datefintrip", "", null, $this);
        $this->columndatefintrip->AutoIncrement(False);
        $this->columndatefintrip->AutoIncrementStep(0);
        $this->columndatefintrip->AutoIncrementSeed(0);
        $this->columndatefintrip->AllowDBNull(True);
        $this->columndatefintrip->ReadOnly(False);
        $this->columndatefintrip->Unique(False);
        $this->columndatefintrip->MaxLength(-1);
        $this->columndatefintrip->Caption("datefintrip");
        $this->columndatefintrip->DefaultValue('<DBNull>');
        $this->Columns()->Add($this->columndatefintrip);
        $this->columnnametrip = new TDataColumn();
        $this->columnnametrip->TDataColumn3("nametrip", "", null, $this);
        $this->columnnametrip->AutoIncrement(False);
        $this->columnnametrip->AutoIncrementStep(0);
    ...
    public function  dateinitripcolumn()
    {
        return $this->columndateinitrip;
    }

    public function  datefintripcolumn()
    {
        return $this->columndatefintrip;
    }
    ...
    public function  Addt_realtripRow($idtrip, $iduser, $isclosed, $dateinitrip, $datefintrip, $nametrip, $nameuser)
    {
        $row = new Row($this);
        ...
        $row->dateinitrip($dateinitrip);
        $row->datefintrip($datefintrip);
        $row->nametrip($nametrip);
        $row->nameuser($nameuser);
        $this->Rows()->Add($row);
        return $row;
...
class t_descrealtripDataTable extends TDataTable
{
    private $columnidrealtripdesc;
    private $columnidrealtrip;
    private $columntime;
    private $columnidtourpolo;
    private $columnnamepolo;
    ...
    private function  InitClass()
    {
        $this->columnidrealtripdesc = new TDataColumn();
        $this->columnidrealtripdesc->TDataColumn3("idrealtripdesc", "", null, $this);
        $this->columnidrealtripdesc->AutoIncrement(True);
        $this->columnidrealtripdesc->AutoIncrementStep(1);
        $this->columnidrealtripdesc->AutoIncrementSeed(1);
        $this->columnidrealtripdesc->AllowDBNull(False);
        $this->columnidrealtripdesc->ReadOnly(False);
        $this->columnidrealtripdesc->Unique(True);
        $this->columnidrealtripdesc->MaxLength(-1);
        $this->columnidrealtripdesc->Caption("idrealtripdesc");
        $this->columnidrealtripdesc->DefaultValue('<DBNull>');
        $this->Columns()->Add($this->columnidrealtripdesc);
        $this->columnidrealtrip = new TDataColumn();
        $this->columnidrealtrip->TDataColumn3(" idrealtrip", "", null, $this);
        $this->columnidrealtrip->AutoIncrement(False);
        $this->columnidrealtrip->AutoIncrementStep(0);
        $this->columnidrealtrip->AutoIncrementSeed(0);
        $this->columnidrealtrip->AllowDBNull(True);
        $this->columnidrealtrip->ReadOnly(False);
        $this->columnidrealtrip->Unique(False);
        $this->columnidrealtrip->MaxLength(-1);
        $this->columnidrealtrip->Caption(" idrealtrip");
        $this->columnidrealtrip->DefaultValue('<DBNull>');
        $this->Columns()->Add($this->columnidrealtrip);
        $this->columntime = new TDataColumn();
        $this->columntime->TDataColumn3("time", "", null, $this);
        $this->columntime->AutoIncrement(False);
        $this->columntime->AutoIncrementStep(0);
        $this->columntime->AutoIncrementSeed(0);
        $this->columntime->AllowDBNull(True);
        $this->columntime->ReadOnly(False);
        $this->columntime->Unique(False);
        $this->columntime->MaxLength(-1);
        $this->columntime->Caption("time");
        $this->columntime->DefaultValue('<DBNull>');
        ...
        $this->columndescpolo->DefaultValue('<DBNull>');
        $this->Columns()->Add($this->columndescpolo);
        $this->DefaultType('t_descrealtripRow');
    }

    public function  __construct()
    {
        parent::__construct();
        $this->SetTableName("t_descrealtrip");
        parent::BeginInit();
        $this->InitClass();
        parent::EndInit();
    }

    public function  idrealtripdesccolumn()
    {
        return $this->columnidrealtripdesc;
    }
    ...
    public function  Addt_descrealtripRow($idrealtrip, $time, $idtourpolo, $namepolo, $descpolo)
    {
        $row = new Row($this);
        $row->idrealtripdesc($this->Rows()->Count() + 1);
        $row->idrealtrip($idrealtrip);
        $row->time($time);
        $row->idtourpolo($idtourpolo);
        $row->namepolo($namepolo);
        $row->descpolo($descpolo);
    ...
    public
    function  GetByidrealtripdesc($idrealtripdesc)
    {
        for ($i = 0;
             $i < $this->Count();
             $i++) {
            if ($this->Row($i)->idrealtripdesc() == $idrealtripdesc) {
                return $this->Row($i);
            }
        }
        return null;
...
class t_commentTableAdapter extends TTableAdapter
{
    ...
    public function    Update($Original_idcomment, $iduser, $topiccomment, $textcomment)
    {
        $this->FUpdateCommand->Parametres()->GetItem(0)->Value($Original_idcomment);
        $this->FUpdateCommand->Parametres()->GetItem(1)->Value($iduser);
        $this->FUpdateCommand->Parametres()->GetItem(2)->Value($topiccomment);
        $this->FUpdateCommand->Parametres()->GetItem(3)->Value($textcomment);
...
class t_tripTableAdapter extends TTableAdapter
{
    ...
    public function    Update($Original_idtrip, $iduser, $nametrip)
    {
        $this->FUpdateCommand->Parametres()->GetItem(0)->Value($Original_idtrip);
        $this->FUpdateCommand->Parametres()->GetItem(1)->Value($iduser);
        $this->FUpdateCommand->Parametres()->GetItem(2)->Value($nametrip);
        $this->FUpdateCommand->Execute();
    }
...
class t_tourpoloTableAdapter extends TTableAdapter
{
    ...
    public function    Update($Original_idtourpolo, $namepolo, $datapolo, $descpolo)
    {
        $this->FUpdateCommand->Parametres()->GetItem(0)->Value($Original_idtourpolo);
        $this->FUpdateCommand->Parametres()->GetItem(1)->Value($namepolo);
        $this->FUpdateCommand->Parametres()->GetItem(2)->Value($datapolo);
        $this->FUpdateCommand->Parametres()->GetItem(3)->Value($descpolo);
        $this->FUpdateCommand->Execute();
    }
...
class t_poloimagesTableAdapter extends TTableAdapter
{
    ...
    public function  __construct()
    {
        ...
        $this->FFillBy->SetSql(" SELECT idimage, pathimage, idtour FROM t_poloimages WHERE idimage = '@param' ");
    ...
    public function    Update($Original_idimage, $pathimage, $idtour)
    {
        $this->FUpdateCommand->Parametres()->GetItem(0)->Value($Original_idimage);
        $this->FUpdateCommand->Parametres()->GetItem(1)->Value($pathimage);
        $this->FUpdateCommand->Parametres()->GetItem(2)->Value($idtour);
        $this->FUpdateCommand->Execute();
    }
...
class t_messagesTableAdapter extends TTableAdapter
{
    ...
    public function  __construct()
    {
        ...
        $this->FDeleteCommand->_construct($this);
        $this->FDeleteCommand->Type('Text');
        $this->FDeleteCommand->FParams->CreateAndAdd("@Original_idmessages", "int", "Input", "iddestuser");
        $this->FUpdateCommand = new TDataCommand();
        $this->Commands()->Add($this->FUpdateCommand);
        ...
        $this->FFillBy1->FParams->CreateAndAdd("@param", "nvarchar", "U?E???  ?@?x?-  ?U?E??o?  ?@3??u?  ???????N?  ??????P?U?E??X?  ???????1?  ??????X?????u?U?E??$?  ?@?   ?'?  ?E???U?E???  ?", "");
    }
...
class t_destripTableAdapter extends TTableAdapter
{
    ...
    public function  __construct()
    {
        ...
        $this->FSelectCommand = new TDataCommand();
        $this->Commands()->Add($this->FSelectCommand);
        $this->FSelectCommand->SetSql("SELECT idtripdes,idtrip,idtourpolo FROM t_destrip");
        $this->FSelectCommand->SetConnection($this->FConnection);
        $this->FSelectCommand->_construct($this);
        $this->FSelectCommand->Type('Text');
        $this->FInsertCommand = new TDataCommand();
        $this->Commands()->Add($this->FInsertCommand);
        $this->FInsertCommand->SetSql("INSERT INTO t_destrip(idtrip,idtourpolo) VALUES ('@idtrip','@idtourpolo')");
        $this->FInsertCommand->SetConnection($this->FConnection);
        $this->FInsertCommand->_construct($this);
        $this->FInsertCommand->Type('Text');
        ...
        $this->FDeleteCommand = new TDataCommand();
        $this->Commands()->Add($this->FDeleteCommand);
        $this->FDeleteCommand->SetSql("DELETE FROM t_destrip WHERE idtripdes = '@Original_idtripdes'");
        $this->FDeleteCommand->SetConnection($this->FConnection);
        $this->FDeleteCommand->_construct($this);
        $this->FDeleteCommand->Type('Text');
        ...
        $this->FUpdateCommand = new TDataCommand();
        $this->Commands()->Add($this->FUpdateCommand);
        $this->FUpdateCommand->SetSql("UPDATE t_destrip SET idtrip='@idtrip',idtourpolo='@idtourpolo' WHERE idtripdes = '@Original_idtripdes'");
        $this->FUpdateCommand->SetConnection($this->FConnection);
        $this->FUpdateCommand->_construct($this);
        $this->FUpdateCommand->Type('Text');
        ...
        $this->FFillByTripID->SetSql(" SELECT idtripdes, idtrip, idtourpolo FROM t_destrip WHERE idtrip = '@param'");
        $this->FFillByTripID->SetConnection($this->FConnection);
        $this->FFillByTripID->_construct($this);
        $this->FFillByTripID->Type('Text');
        $this->FFillByTripID->FParams->CreateAndAdd("@param", "int", "Input", "param");
    ...
    public function  FillByTripID($table, $param)
    {
        $this->FTable = $table;
...
class t_realtripTableAdapter extends TTableAdapter
{
    ...
    public function  __construct()
    {
        ...
        $this->FSelectCommand = new TDataCommand();
        $this->Commands()->Add($this->FSelectCommand);
        $this->FSelectCommand->SetSql("SELECT idrealtrip,idtrip,iduser,isclosed,dateinitrip,datefintrip FROM t_realtrip");
        $this->FSelectCommand->SetConnection($this->FConnection);
        $this->FSelectCommand->_construct($this);
        $this->FSelectCommand->Type('Text');
        $this->FInsertCommand = new TDataCommand();
        $this->Commands()->Add($this->FInsertCommand);
        $this->FInsertCommand->SetSql("INSERT INTO t_realtrip(idtrip,iduser,isclosed,dateinitrip,datefintrip) VALUES ('@idtrip','@iduser','@isclosed','@dateinitrip','@datefintrip')");
        $this->FInsertCommand->SetConnection($this->FConnection);
        $this->FInsertCommand->_construct($this);
        $this->FInsertCommand->Type('Text');
        $this->FInsertCommand->FParams->CreateAndAdd("@idtrip", "int", "Input", "idtrip");
        $this->FInsertCommand->FParams->CreateAndAdd("@iduser", "int", "Input", "iduser");
        $this->FInsertCommand->FParams->CreateAndAdd("@isclosed", "int", "Input", "isclosed");
        $this->FInsertCommand->FParams->CreateAndAdd("@dateinitrip", "", "Input", "dateinitrip");
        $this->FInsertCommand->FParams->CreateAndAdd("@datefintrip", "", "Input", "datefintrip");
        $this->FDeleteCommand = new TDataCommand();
        $this->Commands()->Add($this->FDeleteCommand);
        ...
        $this->FDeleteCommand->_construct($this);
        $this->FDeleteCommand->Type('Text');
        $this->FDeleteCommand->FParams->CreateAndAdd("@Original_idrealtrip", "int", "Input", "idrealtrip");
        $this->FUpdateCommand = new TDataCommand();
        $this->Commands()->Add($this->FUpdateCommand);
        $this->FUpdateCommand->SetSql("UPDATE t_realtrip SET idtrip='@idtrip',iduser='@iduser',isclosed='@isclosed',dateinitrip='@dateinitrip',datefintrip='@datefintrip' WHERE idrealtrip = '@Original_idrealtrip'");
        $this->FUpdateCommand->SetConnection($this->FConnection);
        $this->FUpdateCommand->_construct($this);
        $this->FUpdateCommand->Type('Text');
        $this->FUpdateCommand->FParams->CreateAndAdd("@idrealtrip", "int", "Input", "idrealtrip");
        $this->FUpdateCommand->FParams->CreateAndAdd("@Original_idrealtrip", "int", "Input", "idrealtrip");
        $this->FUpdateCommand->FParams->CreateAndAdd("@idtrip", "int", "Input", "idtrip");
        $this->FUpdateCommand->FParams->CreateAndAdd("@iduser", "int", "Input", "iduser");
        $this->FUpdateCommand->FParams->CreateAndAdd("@isclosed", "int", "Input", "isclosed");
        $this->FUpdateCommand->FParams->CreateAndAdd("@dateinitrip", "", "Input", "dateinitrip");
        $this->FUpdateCommand->FParams->CreateAndAdd("@datefintrip", "", "Input", "datefintrip");
        $this->FFillByID = new TDataCommand();
        $this->OtherCommands()->Add($this->FFillByID);
        $this->FFillByID->SetSql(" SELECT t_realtrip.idrealtrip, t_realtrip.idtrip, t_realtrip.iduser, t_realtrip.isclosed, t_realtrip.dateinitrip, t_realtrip.datefintrip, t_trip.nametrip, t_user.nameuser FROM t_realtrip INNER JOIN t_trip ON t_trip.idtrip = t_realtrip.idtrip INNER JOIN t_user ON t_user.iduser = t_realtrip.iduser WHERE idrealtrip = '@param' OR '@param'='-1'");
        $this->FFillByID->SetConnection($this->FConnection);
        $this->FFillByID->_construct($this);
        $this->FFillByID->Type('Text');
        $this->FFillByID->FParams->CreateAndAdd("@param", "int", "Input", "'param'");
    ...
    public function    Insert($idtrip, $iduser, $isclosed, $dateinitrip, $datefintrip)
    {
        ...
        $this->FInsertCommand->Parametres()->GetItem(3)->Value($dateinitrip);
        $this->FInsertCommand->Parametres()->GetItem(4)->Value($datefintrip);
        $this->FInsertCommand->Execute();
    }
    ...
    public function    Update($idrealtrip, $Original_idrealtrip, $idtrip, $iduser, $isclosed, $dateinitrip, $datefintrip)
    {
        $this->FUpdateCommand->Parametres()->GetItem(0)->Value($idrealtrip);
        $this->FUpdateCommand->Parametres()->GetItem(1)->Value($Original_idrealtrip);
        $this->FUpdateCommand->Parametres()->GetItem(2)->Value($idtrip);
        $this->FUpdateCommand->Parametres()->GetItem(3)->Value($iduser);
        $this->FUpdateCommand->Parametres()->GetItem(4)->Value($isclosed);
        $this->FUpdateCommand->Parametres()->GetItem(5)->Value($dateinitrip);
        $this->FUpdateCommand->Parametres()->GetItem(6)->Value($datefintrip);
        $this->FUpdateCommand->Execute();
    }
...
class t_descrealtripTableAdapter extends TTableAdapter
{
    private $FFillByIDTrip;
    ...
    public function  __construct()
    {
        ...
        $this->FSelectCommand = new TDataCommand();
        $this->Commands()->Add($this->FSelectCommand);
        $this->FSelectCommand->SetSql("SELECT idrealtripdesc, idviaje,time,idtourpolo FROM t_descrealtrip");
        $this->FSelectCommand->SetConnection($this->FConnection);
        $this->FSelectCommand->_construct($this);
        $this->FSelectCommand->Type('Text');
        $this->FInsertCommand = new TDataCommand();
        $this->Commands()->Add($this->FInsertCommand);
        $this->FInsertCommand->SetSql("INSERT INTO t_descrealtrip(idviaje,time,idtourpolo) VALUES ('@idrealtrip','@time','@idtourpolo')");
        $this->FInsertCommand->SetConnection($this->FConnection);
        $this->FInsertCommand->_construct($this);
        $this->FInsertCommand->Type('Text');
        $this->FInsertCommand->FParams->CreateAndAdd("@idrealtrip", "int", "Input", "idrealtrip");
        $this->FInsertCommand->FParams->CreateAndAdd("@time", "int", "Input", "time");
        $this->FInsertCommand->FParams->CreateAndAdd("@idtourpolo", "int", "Input", "idtourpolo");
        $this->FDeleteCommand = new TDataCommand();
        $this->Commands()->Add($this->FDeleteCommand);
        $this->FDeleteCommand->SetSql("DELETE FROM t_descrealtrip WHERE idrealtripdesc = '@Original_idrealtripdesc'");
        $this->FDeleteCommand->SetConnection($this->FConnection);
        $this->FDeleteCommand->_construct($this);
        $this->FDeleteCommand->Type('Text');
        $this->FDeleteCommand->FParams->CreateAndAdd("@Original_idrealtripdesc", "int", "Input", "idrealtripdesc");
        $this->FUpdateCommand = new TDataCommand();
        $this->Commands()->Add($this->FUpdateCommand);
        $this->FUpdateCommand->SetSql("UPDATE t_descrealtrip SET idrealtrip='@idviaje',time='@time',idtourpolo='@idtourpolo' WHERE idrealtripdesc = '@Original_idrealtripdesc'");
        $this->FUpdateCommand->SetConnection($this->FConnection);
        $this->FUpdateCommand->_construct($this);
        $this->FUpdateCommand->Type('Text');
        $this->FUpdateCommand->FParams->CreateAndAdd("@idviaje", "int", "Input", "idviaje");
        $this->FUpdateCommand->FParams->CreateAndAdd("@Original_idrealtripdesc", "int", "Input", "idrealtripdesc");
        $this->FUpdateCommand->FParams->CreateAndAdd("@idrealtripdesc", "int", "Input", "idrealtripdesc");
        $this->FUpdateCommand->FParams->CreateAndAdd("@time", "int", "Input", "time");
        $this->FUpdateCommand->FParams->CreateAndAdd("@idtourpolo", "int", "Input", "idtourpolo");
        $this->FFillByIDTrip = new TDataCommand();
        $this->OtherCommands()->Add($this->FFillByIDTrip);
        $this->FFillByIDTrip->SetSql(" SELECT t_descrealtrip.idrealtripdesc, t_descrealtrip.idrealtrip, t_descrealtrip.time, t_descrealtrip.idtourpolo, t_tourpolo.namepolo, t_tourpolo.descpolo FROM t_descrealtrip INNER JOIN t_tourpolo ON t_tourpolo.idtourpolo = t_descrealtrip.idtourpolo WHERE idrealtrip = '@param' OR '@param' = '-1'");
        $this->FFillByIDTrip->SetConnection($this->FConnection);
        $this->FFillByIDTrip->_construct($this);
        $this->FFillByIDTrip->Type('Text');
        $this->FFillByIDTrip->FParams->CreateAndAdd("@param", "int", "Input", "param");
    }

    public function    Insert($idrealtripdesc, $time, $idtourpolo)
    {

        $this->FInsertCommand->Parametres()->GetItem(0)->Value($idrealtripdesc);
        $this->FInsertCommand->Parametres()->GetItem(1)->Value($time);
        $this->FInsertCommand->Parametres()->GetItem(2)->Value($idtourpolo);
    return    $this->FInsertCommand->Execute();
    }

    public function    Delete($Original_idrealtripdesc)
    {
        $this->FDeleteCommand->Parametres()->GetItem(0)->Value($Original_idrealtripdesc);
        $this->FDeleteCommand->Execute();
    }

    public function    Update($idrealtripdesc, $Original_idrealtripdesc, $idrealtripdesc, $time, $idtourpolo)
    {
        $this->FUpdateCommand->Parametres()->GetItem(0)->Value($idrealtripdesc);
        $this->FUpdateCommand->Parametres()->GetItem(1)->Value($Original_idrealtripdesc);
        $this->FUpdateCommand->Parametres()->GetItem(2)->Value($idrealtripdesc);
        $this->FUpdateCommand->Parametres()->GetItem(3)->Value($time);
        $this->FUpdateCommand->Parametres()->GetItem(4)->Value($idtourpolo);
        $this->FUpdateCommand->Execute();
    }

    public function  FillByIDTrip($table, $param)
    {
        $this->FTable = $table;
        $this->FFillByIDTrip->Parametres()->GetItem(0)->Value($param);
        $this->FFillByIDTrip->Execute();
    }
}
...
class  DSCubaMyWay extends TDataSet
{
    ...
    public function  __construct()
    {
        ...
        $this->Ft_destrip = new t_destripDataTable();
        $this->Ft_destrip->construct("t_destrip",$this);
        parent::Tables()->Add($this->Ft_destrip);
        $this->Ft_realtrip = new t_realtripDataTable();
        $this->Ft_realtrip->construct("t_realtrip",$this);
        parent::Tables()->Add($this->Ft_realtrip);
        $this->Ft_descrealtrip = new t_descrealtripDataTable();
        $this->Ft_descrealtrip->construct("t_descrealtrip",$this);
        parent::Tables()->Add($this->Ft_descrealtrip);
    ...
    public function  t_destrip()
    {
        return $this->Ft_destrip;
    }
    ...
    public function  t_descrealtrip()
    {
        return $this->Ft_descrealtrip;
    }
}
